...
export default function App() {
  const [view, setView] = useState(() => {
    if (typeof location !== "undefined") {
      if (location.pathname.includes("thank-you") || location.hash === "#thank-you") return "thank-you";
      if (location.hash === "#properties") return "properties";
      if (location.hash === "#blog") return "blog";
    }
    return "home";
  });
  ...
  const goBlog = () => { setMenu(false); setView("blog"); window.scrollTo(0, 0); if (history.replaceState) history.replaceState(null, "", "#blog"); };
  const navAnchor = (hash) => (ev) => { ev.preventDefault(); setMenu(false); if (view !== "home") { goHome(); setTimeout(() => document.querySelector(hash)?.scrollIntoView({ behavior: "smooth" }), 60); } else { document.querySelector(hash)?.scrollIntoView({ behavior: "smooth" }); } };
...
useEffect(() => {
  ...
  <a href="#top" className="brand" aria-label="NB Realtor - where trust meets address"
     onClick={(e) => { if (view !== "home") { e.preventDefault(); goHome(); } setMenu(false); }}>
  ...
  <nav className={`lk${menu ? " open" : ""}`} id="lk">
    <a className="l" href="#about" onClick={(e) => { navAnchor("#about")(e); setMenu(false); }}>About</a>
    <a className="l" href="#approach" onClick={(e) => { navAnchor("#approach")(e); setMenu(false); }}>Approach</a>
    <a className="l" href="#properties" onClick={(e) => { e.preventDefault(); goProperties(); setMenu(false); }}>Properties</a>
    <a className="l" href="#reviews" onClick={(e) => { navAnchor("#reviews")(e); setMenu(false); }}>Reviews</a>
    <a className="l" href="#blog" onClick={(e) => { e.preventDefault(); goBlog(); setMenu(false); }}>Blog</a>
    <a className="l" href="#emi" onClick={(e) => { navAnchor("#emi")(e); setMenu(false); }}>Calculator</a>
    <span className="nav-ctas"><button className="btn btn-line nav-cta nav-list" onClick={() => { openSell(); setMenu(false); }}>List Property</button><button className="btn btn-solid nav-cta" onClick={() => { openBook(); setMenu(false); }}>Request a Consultation</button></span>
  </nav>
  <button className={`burger${menu ? " is-open" : ""}`} id="burger" aria-label="Menu" aria-expanded={menu} onClick={() => setMenu((m) => !m)}><span></span><span></span><span></span></button>
</header>
      ...
      <div hidden={view !== "blog"}><Blog onBook={openBook} active={view === "blog"} /></div>
