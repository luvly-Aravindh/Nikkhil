...
import { bg, EMAIL_RE, validatePhone, phoneError } from "./lib.js";
...
export function BookModal({ open, step, setStep, onClose }) {
  const [f, setF] = useState({ name: "", phone: "", dial: "+91", email: "", type: "To live in", budget: "\u20b93.5 Cr to \u20b95 Cr", area: "", msg: "" });
  ...
  async function submit(ev) {
    ...
    if (!f.phone) er.phone = "Please enter your phone number."; else if (!validatePhone(f.dial, f.phone)) er.phone = phoneError(f.dial);
    if (f.email.trim() && !EMAIL_RE.test(f.email.trim())) er.email = "Enter a valid email address.";
    setE(er); if (Object.keys(er).length) return;
    setBusy(true);
    try {
      await submitLead({ form_type: "consultation", subject: `New Lead - Nikhil Bora Consultation`, name: f.name.trim(), phone: `${f.dial}${f.phone}`, email: f.email.trim(), buying_for: f.type, budget: f.budget, preferred_area: f.area, message: f.msg.trim() });
    } catch (err2) { console.error("Lead submit failed:", err2); }
    setBusy(false);
    setWaLink(wa(["Hi Nikhil, I would like to book a consultation.", "", "Name: " + f.name.trim(), "Phone: " + f.dial + f.phone, f.email.trim() ? "Email: " + f.email.trim() : "", "Buying for: " + f.type, "Budget: " + f.budget, "Preferred area: " + f.area, f.msg.trim() ? "Looking for: " + f.msg.trim() : ""]));
    setStep(2);
  }
          ...
          <div style={{ display: step === 1 ? "block" : "none" }}>
            ...
            <form className="mform" noValidate onSubmit={submit}>
              <div className="frow">
                ...
                <div className="fld"><label htmlFor="m-phone">Phone / WhatsApp</label><PhoneField id="m-phone" dial={f.dial} value={f.phone} onDial={set("dial")} onChange={set("phone")} invalid={!!e.phone} />{e.phone && <small style={err}>{e.phone}</small>}</div>
              </div>
...
export function SellModal({ open, onClose }) {
  const [f, setF] = useState({ name: "", phone: "", dial: "+91", project: "", loc: "", carpet: "", parking: "None", furnish: "Unfurnished", notes: "" });
  const [e, setE] = useState({});
  const [busy, setBusy] = useState(false);
  const set = (k) => (v) => { const val = typeof v === "string" ? v : v.target.value; setF((d) => ({ ...d, [k]: val })); setE((x) => (x[k] ? { ...x, [k]: undefined } : x)); };
  ...
  async function submit(ev) {
    ...
    setBusy(true);
    try {
      await submitLead({ form_type: "sell_listing", subject: `New Listing - Nikhil Bora`, name: f.name.trim(), phone: `${f.dial}${f.phone}`, project: f.project.trim(), location: f.loc, carpet_sqft: f.carpet.trim(), parking: f.parking, furnishing: f.furnish, notes: f.notes.trim() });
    } catch (err2) { console.error("Lead submit failed:", err2); }
    setBusy(false);
    const waUrl = wa(["Hi Nikhil, I would like to list my property.", "", "Name: " + f.name.trim(), "Phone: " + f.dial + f.phone, "Building / project: " + f.project.trim(), f.loc ? "Location: " + f.loc : "", f.carpet.trim() ? "Carpet area: " + f.carpet.trim() + " sq ft" : "", "Parking: " + f.parking, "Furnishing: " + f.furnish, f.notes.trim() ? "Notes: " + f.notes.trim() : ""]);
    
    // Redirect to the new thank-you.html page
        ...
        <div className="pad">
          ...
          <form noValidate onSubmit={submit}>
            <div className="frow">
              ...
              <div className="fld"><label htmlFor="s-phone">Phone / WhatsApp</label><PhoneField id="s-phone" dial={f.dial} value={f.phone} onDial={set("dial")} onChange={set("phone")} invalid={!!e.phone} />{e.phone && <small style={err}>{e.phone}</small>}</div>
            </div>
            <div className="frow">
